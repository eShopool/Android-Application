 ...
 * @version 1.0.1
 * @since 27/9/2019
 * This class is used to load soap balance.
 */
public class LoadSOAPBalance
{
    private int[] weight;
    private int[] effectiveWeight;
    private int[] currentWeight;
    static final String[] SOAPServers = {"192.168.0.77","192.168.0.209"};

    static final String ping_regex = "(time=)(.*)(ms)";


    public LoadSOAPBalance()
    {

        int numOfSlave = SOAPServers.length;
        ...
        for(int i = 0; i< numOfSlave; i++) {
            latency[i] = pingIP(SOAPServers[i]);
            sum += latency[i];
            currentWeight[i] = 0;
        }
    ...
    public String getIP()
    {
        ...
        int maxIndex = 0;
        int totalWeight = 0;
        for(int i = 0; i < SOAPServers.length; i++)
        {
